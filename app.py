...
def generateWordCloudTitle(data):
    ...
    wordCloud = v.get_feature_names_out()
    return wordCloud
...
def rankRecommendations(predSorted, filename):
    ...
    return finalrecodf[['Title of the Article','Authors','Published Journal','Abstract of the article',]]
...
def create_recommendation_table(df):
    class RecommendationTable(Table):
        classes = ['Table', 'table-striped', 'table-bordered', 'table-condensed']
        index = Col('Rank', show=True)
        title = Col('Title of Paper')
        author = Col('Authors')
        abstract = Col('Abstract')
        show_similar = LinkCol('Show Similar', 'showSimilar', url_kwargs=dict(index='index'))
    items = []
    i=1
    for index, row in df.iterrows():
        items.append(dict(
            index=i,
            title=row['Title of the Article'],
            author=row['Authors'],
            abstract=row['Abstract of the article']
        ))
        i+=1
    table = RecommendationTable(items)
    return table
def create_similar_table(df):
    class SimilarTable(Table):
        classes = ['Table', 'table-striped', 'table-bordered', 'table-condensed']
        index = Col('Index', show=True)
        title = Col('Title of Paper')
        author = Col('Authors')
    items = []
    for index, row in df.iterrows():
...
def showSimilar(index):
    ...
    if(len(result) != 0):
        ...
        return render_template('table1.html', table=table_html)
    else:
